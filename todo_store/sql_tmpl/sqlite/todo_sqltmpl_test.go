...
import (
	...
	"github.com/senomas/gosvc_todo/todo_store/sql_tmpl/sqlite"
...
func TestTodoSqlTemplateSqlite(t *testing.T) {
	...
	t.Run("FindTodoTotal no filter", func(t *testing.T) {
		qry, args := tl.FindTodoTotal(todo_store.TodoFilter{})
	...
	t.Run("FindTodoTotal where id =", func(t *testing.T) {
		qry, args := tl.FindTodoTotal(todo_store.TodoFilter{
			ID: store.FilterInt64{Value: 100, Op: store.OP_EQ},
		})
	...
	t.Run("FindTodoTotal where title =", func(t *testing.T) {
		qry, args := tl.FindTodoTotal(todo_store.TodoFilter{
			Title: store.FilterString{Value: "foo", Op: store.OP_EQ},
		})
	...
	t.Run("FindTodoTotal where title like", func(t *testing.T) {
		qry, args := tl.FindTodoTotal(todo_store.TodoFilter{
			Title: store.FilterString{Value: "foo", Op: store.OP_LIKE},
		})
