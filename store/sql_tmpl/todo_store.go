...
func (t *TodoStoreTmpl) CreateTodo(ctx context.Context, title string) (*store.Todo, error) {
	if db, ok := ctx.Value(svc_store.StoreCtxDB).(*sql.DB); ok {
		todo := store.Todo{Title: title}
		qry, args := todoStoreTemplateImpl.InsertTodo(&todo)
		slog.Debug("CreateTodo", "qry", qry, "args", &svc_store.JsonLogValue{V: args})
...
func (t *TodoStoreTmpl) UpdateTodo(ctx context.Context, todo store.Todo) error {
	if db, ok := ctx.Value(svc_store.StoreCtxDB).(*sql.DB); ok {
		...
		slog.Debug("UpdateTodo", "qry", qry, "args", &svc_store.JsonLogValue{V: args})
...
func (t *TodoStoreTmpl) DeleteTodoByID(ctx context.Context, id int64) error {
	if db, ok := ctx.Value(svc_store.StoreCtxDB).(*sql.DB); ok {
		...
		slog.Debug("DeleteTodoByID", "qry", qry, "args", &svc_store.JsonLogValue{V: args})
		_, err := db.ExecContext(ctx, qry, args...)
		return err
	}
	return errCtxNoDB
...
func (t *TodoStoreTmpl) GetTodoByID(ctx context.Context, id int64) (*store.Todo, error) {
	if db, ok := ctx.Value(svc_store.StoreCtxDB).(*sql.DB); ok {
		...
		slog.Debug("GetTodoByID", "qry", qry, "args", &svc_store.JsonLogValue{V: args})
...
func (*TodoStoreTmpl) FindTodo(ctx context.Context, filter store.TodoFilter, skip int64, count int) ([]*store.Todo, int64, error) {
	if db, ok := ctx.Value(svc_store.StoreCtxDB).(*sql.DB); ok {
		...
		slog.Debug("FindTodoTotal", "qry", qry, "args", &svc_store.JsonLogValue{V: args})
		...
		slog.Debug("FindTodo", "qry", qry, "args", &svc_store.JsonLogValue{V: args})
